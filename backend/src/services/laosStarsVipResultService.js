...
const LAOS_STAR_VIP_PROVIDER_NAME = 'Lao Stars VIP Official';
const LAOS_STAR_VIP_MARKET_ID = 'lao_star_vip';
const LAOS_STAR_VIP_MARKET_NAME = 'ลาวสตาร์ VIP';
const LAOS_STAR_VIP_SITE_URL = 'https://laostars-vip.com/';
const LAOS_STAR_VIP_RESULT_URL = 'https://api.laostars-vip.com/result';
const LAOS_STAR_VIP_HISTORY_URL = 'https://api.laostars-vip.com/history';
const LAOS_STAR_VIP_TIMEOUT_MS = Number(process.env.LAOS_STAR_VIP_TIMEOUT_MS || 15000);

const http = axios.create({
  timeout: LAOS_STAR_VIP_TIMEOUT_MS,
  headers: {
...
const buildSnapshot = ({ roundCode, results, publishedAt, rawPayload }) => {
  ...
  return {
    lotteryCode: LAOS_STAR_VIP_MARKET_ID,
    feedCode: LAOS_STAR_VIP_MARKET_ID,
    marketName: LAOS_STAR_VIP_MARKET_NAME,
    roundCode,
    headline: threeTop,
    firstPrize: digit5,
    threeTop,
    ...
    sourceUrl: LAOS_STAR_VIP_SITE_URL,
    rawPayload
  };
};

const fetchCurrentSnapshot = async () => {
  const response = await http.get(LAOS_STAR_VIP_RESULT_URL);
...
const fetchHistorySnapshots = async () => {
  const response = await http.get(LAOS_STAR_VIP_HISTORY_URL);
  ...
  return rows
    .map((row) => buildSnapshot({
      ...
      results: row?.results,
      publishedAt: parseBangkokDateTime(`${stringValue(row?.lotto_date)} 22:00:00`),
      rawPayload: row
    }))
    .filter(Boolean);
};

const fetchLaosStarsVipSnapshots = async ({ limit = 10 } = {}) => {
...
const fetchLatestLaosStarsVipSnapshot = async () => {
  const snapshots = await fetchLaosStarsVipSnapshots({ limit: 1 });
...
module.exports = {
  ...
  LAOS_STAR_VIP_SITE_URL,
  fetchLaosStarsVipSnapshots,
